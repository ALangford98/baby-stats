...
describe('ReportScreen', () => {
  it('shows the current report text', () => {
    render(<ReportScreen day={baseDay} activities={ACTIVITIES} settings={settingsNoKey} onGenerateAi={vi.fn()} aiLoading={false} aiError={null} onContinue={vi.fn()} />);
  ...
  it('shows the "Generate with AI" button and calls onGenerateAi when a key is configured', async () => {
    ...
    render(<ReportScreen day={baseDay} activities={ACTIVITIES} settings={settingsWithKey} onGenerateAi={onGenerateAi} aiLoading={false} aiError={null} onContinue={vi.fn()} />);
  ...
  it('keeps showing the offline report and an error message if AI generation failed', () => {
    render(<ReportScreen day={baseDay} activities={ACTIVITIES} settings={settingsWithKey} onGenerateAi={vi.fn()} aiLoading={false} aiError="Anthropic API error: 401" onContinue={vi.fn()} />);
    expect(screen.getByText('Offline report text')).toBeInTheDocument();
  ...
  it('calls onContinue when the user is done reviewing', async () => {
    ...
    render(<ReportScreen day={baseDay} activities={ACTIVITIES} settings={settingsNoKey} onGenerateAi={vi.fn()} aiLoading={false} aiError={null} onContinue={onContinue} />);
